...
discIrrSH=4;     %zones of different irradiation for superheater
superheaterInj=2;    %1 if no SH injection, 2 if SH injection
...
lengthTubeHorSH=abs(round((lengthTubeSH/superheaterInj)/discIrrSH*sind(orientation)))
lengthTubeHorSHPowerBlock=abs(round(120*sind(orientation)))
...
% loop for superheater Injection
for l=1:superheaterInj
...
(l-1)*lengthTubeSH/2*sind(orientation)
...
checkLengthDistortion=ceil((lengthDistortion-(lengthTubeSH/superheaterInj)/2)/((lengthTubeSH/superheaterInj)/discIrrSH));
discDistortion=ceil(lengthDistortion/(lengthTubeSH/superheaterInj)*discIrrSH);
%
%
%
if (orientation >= 180)
...
% else
    ...
    for i=2:(dims1-2)/4+1
     ...
     dummystring=round(startingTimeDistortion+(i-2)*lengthTubeHorSH/velocityDistortion+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion);
    dummystring1=round(startingTimeDistortion+(i-2)*lengthTubeHorSH/velocityDistortion+deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion);
    S2((i-2)*2+2+(j-1)*((((dims1-2)/4+1)-2)*2+2),1)={num2str(dummystring)};
    S2((i-2)*2+3+(j-1)*((((dims1-2)/4+1)-2)*2+2),1)={num2str(dummystring1)};
    end
    end
    ...
    for j=1:nDistortion
         for i=1:(dims1-6)/4+1
     dummystring=round(startingTimeDistortion+(i-1)*lengthTubeHorSH/velocityDistortion+deltaTimeDistortion+deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)
     dummystring1=round(startingTimeDistortion+(i-1)*lengthTubeHorSH/velocityDistortion+deltaTimeDistortion+2*deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)
    ...
    for i=2:discIrrSH+1
        for j=1:length(S2(:,1))
        if (str2num(S2{j,1})>=round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+deltaTime+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)) && (str2num(S2{j,1})<=round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+deltaTime+deltaTimeDistortion+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion))
            S2(j,i)={num2str(Irr-deltaIrr)};
            round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+deltaTime+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)
            round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+deltaTime+deltaTimeDistortion+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)
        end
        end
    end
    end
...
%
    ...
    for i=2:(dims1-2)/4+1
     ...
     dummystring=round(startingTimeDistortion+(i-2)*lengthTubeHorSH/velocityDistortion+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion)
    dummystring1=round(startingTimeDistortion+(i-2)*lengthTubeHorSH/velocityDistortion+deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion)
    S2((i-2)*2+2+(j-1)*((((dims1-2)/4+1)-2)*2+2),1)={num2str(dummystring)};
    S2((i-2)*2+3+(j-1)*((((dims1-2)/4+1)-2)*2+2),1)={num2str(dummystring1)};
    end
    end
    ...
    for j=1:nDistortion
         for i=1:(dims1-6)/4+1
     dummystring=round(startingTimeDistortion+(i-1)*lengthTubeHorSH/velocityDistortion+deltaTimeDistortion+deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion)
     dummystring1=round(startingTimeDistortion+(i-1)*lengthTubeHorSH/velocityDistortion+deltaTimeDistortion+2*deltaTime+(j-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion)
     S2((i-1)*2+2+nDistortion*(discIrrSH*2)+(j-1)*discIrrSH*2,1)={num2str(dummystring)};
    S2((i-1)*2+3+nDistortion*discIrrSH*2+(j-1)*discIrrSH*2,1)={num2str(dummystring1)};
         end
    end
    ...
    S2(1:end,2:dims2)={num2str(Irr)};
        ...
        for i=2:discIrrSH+1
        for j=1:length(S2(:,1))
        if (str2num(S2{j,1})>=round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion+deltaTime+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion)) && (str2num(S2{j,1})<=round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion+deltaTime+deltaTimeDistortion+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion));
            S2(j,discIrrSH+3-i)={num2str(Irr-deltaIrr)};
            (l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion;
            startingTimeDistortion;
            ((i-2)*lengthTubeHorSH)/velocityDistortion;
            lengthTubeHorSHPowerBlock/velocityDistortion;
            deltaTime;
            (h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion);
            round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion+deltaTime+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion);
            round(startingTimeDistortion+((i-2)*lengthTubeHorSH)/velocityDistortion+lengthTubeHorSHPowerBlock/velocityDistortion+deltaTime+deltaTimeDistortion+(h-1)*(diffLengthDistortion/velocityDistortion+deltaTimeDistortion)+(l-1)*lengthTubeSH/2*sind(orientation)/velocityDistortion);
        end
        end
    end
        end
...
%% create new txt file
    ...
    name =char(strcat('SuperheaterLoop', num2str(k),'_', num2str(l)));               % name of file
    pthstr ='C:/Work/Clara_Solar_InputData/InputIrradiation';    % path of file
...
fprintf(fid,'%s\n',['#Collector number' num2str(k)]);
fprintf(fid,'%s\n',['#Date of creation:' date]);
fprintf(fid,'%s\n',['#(lengthTubeSH/superheaterInj)=' num2str((lengthTubeSH/superheaterInj)) ' discIrrSH=' num2str(discIrrSH) ' lengthDistortion=' num2str(lengthDistortion) ' velocityDistortion=' num2str(velocityDistortion) ' endTimeSim=' num2str(endTimeSim) ' deltaTimeDistortion=' num2str(deltaTimeDistortion) ' deltaTime=' num2str(deltaTime) ' Irr=' num2str(Irr) ' deltaIrr=' num2str(deltaIrr) ' startingTimeDistortion=' num2str(startingTimeDistortion) ]);
